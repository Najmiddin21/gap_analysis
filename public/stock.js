...
//     try {
//         const response = await fetch(`http://localhost:3001/gap-analysis?ticker=${ticker}`);
...
//         data.forEach(row => {
//             let line = `${row.Gap.padEnd(15, ' ')} ${String(row["Total Days"]).padEnd(14, ' ')} ${String(row["Closed Down"]).padEnd(14, ' ')} ${String(row["Closed Up"]).padEnd(10, ' ')}\n;`
//             if (row.Gap.includes("up")) {
//                 gapUpSection += line;
//             } else {
//                 gapDownSection += line;
